...
class FeatureImporter(Transformation):
    _name_ = "import"
    @beartype
    def __init__(self, iterable: ListConfig[str | DictConfig[str, str]] | None = None, feature: str | None = None, transform_stage: str | None = None):
        """
        Parameters
        ----------
        ...
        if feature is not None:
            features_to_import.append((feature, transform_stage)  if ":" not in feature else feature.split(":"))
        else:
        ...
        self.iterable: list[PromiseValue] = [
    ...
    def execute(self) -> AnyArray | list[AnyArray]:
        if len(self.iterable) == 1:
            return self.iterable[0]._get_value()
        else:
            imported_list = [promise_value._get_value() for promise_value in self.iterable]
