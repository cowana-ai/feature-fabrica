...
class DateTimeDifference(Transformation):
    @beartype
    def __init__(self, initial_datetime: str | None = None, end_datetime: str | None = None, compute_unit: str | None = None, convert_datetime: str | None = None):
        super().__init__()
    ...
    @beartype
    def execute(self, data: StrArray | DateTimeArray) -> NumericArray | DateTimeArray | StrArray:
        ...
        result = data - self.initial_datetime if self.initial_datetime else self.end_datetime - data
        if self.compute_unit:
            result = result.astype(f'timedelta64[{self.compute_unit}]')

        if self.convert_datetime:
            result = result.astype(self.convert_datetime)
